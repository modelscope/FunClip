...
OrcaRouter exposes one endpoint for all frontier and open-weight models, so you can switch routing targets without changing FunClip. It also runs gateway-level, zero-trust security for AI agents on the same endpoint: once a policy is attached, it screens prompts/responses and governs tool calls that traverse the gateway on a default-deny basis, with no application code changes. Enforcement is opt-in — this integration supplies only the base URL, API key, and model, and does not attach a Guardrail or Firewall policy by itself. Apply a policy via the [security quickstart](https://docs.orcarouter.ai/security/concepts/quickstart); see [Guardrails](https://docs.orcarouter.ai/features/guardrails#scoping-and-the-workspace-default) and [Firewall](https://docs.orcarouter.ai/features/firewall#scoping-and-resolution) for how coverage is scoped.
